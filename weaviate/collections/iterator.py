...
from weaviate.collections.classes.internal import _Object, ReturnProperties, _QueryReturn
...
class _ObjectIterator(Generic[Properties], Iterable[_Object[Properties]]):
    def __init__(
        ...
        fetch_objects_query: Callable[
            [int, Optional[UUID], Optional[MetadataQuery]], _QueryReturn[Properties]
        ],
    ...
    def __next__(self) -> _Object[Properties]:
        if len(self.__iter_object_cache) == 0:
            return_ = self.__query(
                ITERATOR_CACHE_SIZE,
                self.__iter_object_last_uuid,
                self.__meta,
            )
            self.__iter_object_cache = return_.objects
            if len(self.__iter_object_cache) == 0:
                raise StopIteration
...
class _ObjectIteratorAsync(Generic[Properties], AsyncIterable[_Object[Properties]]):
    def __init__(
        self,
        fetch_objects_query: Callable[
            [int, Optional[UUID], Optional[MetadataQuery]],
            Coroutine[Any, Any, _QueryReturn[Properties]],
        ],
        return_metadata: Optional[MetadataQuery],
        return_properties: Optional[ReturnProperties[Properties]],
    ) -> None:
    ...
    def __aiter__(self) -> AsyncIterator[_Object[Properties]]:
        self.__iter_object_cache = []
        self.__iter_object_last_uuid = None
        return self

    async def __anext__(self) -> _Object[Properties]:
        if len(self.__iter_object_cache) == 0:
            return_ = await self.__query(
                ITERATOR_CACHE_SIZE,
                self.__iter_object_last_uuid,
                self.__meta,
            )
