...
from weaviate.collections.query import (
    _GenerateCollection,
    _GenerateCollectionAsync,
    _GroupByCollection,
    _GroupByCollectionAsync,
    _QueryCollection,
    _QueryCollectionAsync,
)
from weaviate.collections.iterator import _ObjectIterator, _ObjectIteratorAsync
from weaviate.collections.tenants import _Tenants, _TenantsAsync
...
class _Collection(_CollectionBase, Generic[Properties]):
    ...
    def with_tenant(self, tenant: Optional[str] = None) -> "_Collection":
        ...
        """
        return _Collection[Properties](
            self._connection, self.name, self.__consistency_level, tenant
        )
    ...
    def with_consistency_level(
        ...
        """
        return _Collection[Properties](
            self._connection, self.name, consistency_level, self.__tenant
        )
...
class _CollectionAsync(_CollectionBase, Generic[Properties]):
    def __init__(
        self,
        ...
        # )
        """This namespace includes all the aggregate methods available to you when using Weaviate's aggregation group-by capabilities."""
        self.config = _ConfigCollectionAsync(self._connection, name)
        """This namespace includes all the CRUD methods available to you when modifying the configuration of the collection in Weaviate."""
        self.data = _DataCollectionAsync[Properties](
            connection, name, consistency_level, tenant, type_
        )
        """This namespace includes all the CUD methods available to you when modifying the data of the collection in Weaviate."""
        self.generate = _GenerateCollectionAsync(connection, name, consistency_level, tenant, type_)
        """This namespace includes all the querying methods available to you when using Weaviate's generative capabilities."""
        self.query_group_by = _GroupByCollectionAsync(
            connection, name, consistency_level, tenant, type_
        )
        """This namespace includes all the querying methods available to you when using Weaviate's querying group-by capabilities."""
        self.query = _QueryCollectionAsync[Properties](
            connection, name, self.data, consistency_level, tenant, type_
        )
        """This namespace includes all the querying methods available to you when using Weaviate's standard query capabilities."""
    ...
    def with_tenant(self, tenant: Optional[str] = None) -> "_CollectionAsync":
        """Use this method to return a collection object specific to a single tenant.
        ...
        Arguments:
            `tenant`
                The name of the tenant to use.
        """
        return _CollectionAsync[Properties](
            self._connection, self.name, self.__consistency_level, tenant
        )
    ...
    #     total = loop.run_until_complete(self.aggregate.over_all(total_count=True)).total_count
    #     assert total is not None
    #     return total
    ...
    def iterator(
        ...
        return_metadata: Optional[MetadataQuery] = None,
        return_properties: Optional[PROPERTIES] = None,
    ) -> _ObjectIteratorAsync[Properties]:
        ...
    ...
    def iterator(
        ...
        *,
        return_properties: Type[TProperties],
    ) -> _ObjectIteratorAsync[TProperties]:
        ...
    ...
    def iterator(
        ...
        return_metadata: Optional[MetadataQuery] = None,
        return_properties: Optional[Union[PROPERTIES, Type[TProperties]]] = None,
    ) -> Union[_ObjectIteratorAsync[Properties], _ObjectIteratorAsync[TProperties]]:
        """Use this method to return an iterator over the objects in the collection.
        ...
        if is_typeddict(return_properties):
            return_properties = cast(Type[TProperties], return_properties)
            return _ObjectIteratorAsync[TProperties](
                lambda limit, alpha, meta: self.query.fetch_objects(
                    limit=limit,
                    after=alpha,
                    return_metadata=meta,
        ...
        if return_properties is None and self.__type is not None:
            _type = cast(Type[Properties], self.__type)
            return _ObjectIteratorAsync[Properties](
                lambda limit, alpha, meta: self.query.fetch_objects(
                    limit=limit,
                    after=alpha,
                    return_metadata=meta,
