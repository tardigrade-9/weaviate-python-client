...
class _BatchGRPC(_BaseGRPC):
    ...
    def objects(self, objects: List[_BatchObject]) -> BatchObjectReturn:
        ...
        weaviate_objs: List[batch_pb2.BatchObject] = [
            batch_pb2.BatchObject(
                ...
                properties=_translate_properties_from_python_to_grpc(obj.properties, False),
                tenant=obj.tenant,
            )
            for obj in objects
        ]
    ...
    def __send_batch(self, batch: List[batch_pb2.BatchObject]) -> Dict[int, str]:
        ...
        try:
            ...
            res, _ = stub.BatchObjects.with_call(
...
class _BatchGRPCAsync(_BaseGRPC):
    """This class is used to insert multiple objects into Weaviate using the gRPC API.
    ...
    async def objects(self, objects: List[_BatchObject]) -> BatchObjectReturn:
        """Insert multiple objects into Weaviate through the gRPC API.
        ...
        start = time.time()
        errors = await self.__send_batch(weaviate_objs)
        elapsed_time = time.time() - start
    ...
    async def __send_batch(self, batch: List[batch_pb2.BatchObject]) -> Dict[int, str]:
        stub = self._connection.grpc_stub()
        assert stub is not None
...
def _translate_properties_from_python_to_grpc(
    data: Dict[str, Any], clean_props: bool
        ...
        if isinstance(val, _Reference):
            if val.is_multi_target:
                ...
                single_target.append(
                    batch_pb2.BatchObject.SingleTargetRefProps(uuids=val.uuids_str, prop_name=key)
                )
        elif isinstance(val, dict):
            parsed = _translate_properties_from_python_to_grpc(val, clean_props)
            ...
            object_array_properties.append(
                base_pb2.ObjectArrayProperties(
                    values=[
                        ...
                        for v in val
                        if (parsed := _translate_properties_from_python_to_grpc(v, clean_props))
                    ],
                    prop_name=key,
                )
            )
