...
class _TenantsAsync:
    ...
    async def create(self, tenants: List[Tenant]) -> None:
        ...
        path = "/schema/" + self.__name + "/tenants"
        try:
            response = await self.__connection.post(path=path, weaviate_object=loaded_tenants)
    ...
    async def remove(self, tenants: List[str]) -> None:
        ...
        path = "/schema/" + self.__name + "/tenants"
        try:
            response = await self.__connection.delete(path=path, weaviate_object=tenants)
        except HttpxConnectionError as conn_err:
            raise WeaviateConnectionException(
                f"Collection tenants may not have been deleted for {self.__name}"
            ) from conn_err
    ...
    async def get(self) -> Dict[str, Tenant]:
        ...
        path = "/schema/" + self.__name + "/tenants"
        try:
            response = await self.__connection.get(path=path)
        except HttpxConnectionError as conn_err:
            raise WeaviateConnectionException(
                f"Could not get collection tenants for {self.__name}"
            ) from conn_err
    ...
    async def update(self, tenants: List[Tenant]) -> None:
        ...
        path = "/schema/" + self.__name + "/tenants"
        try:
            response = await self.__connection.put(path=path, weaviate_object=loaded_tenants)
        except HttpxConnectionError as conn_err:
            raise WeaviateConnectionException(
                f"Collection tenants may not have been updated properly for {self.__name}"
            ) from conn_err
